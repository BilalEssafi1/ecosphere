...
- The logout functionality was not clearing all relevant authentication cookies, such as csrftoken and sessionid, resulting in inconsistent user sessions after logout. The issue was caused by incomplete cookie removal logic, which left behind critical session data, especially in browsers where these cookies were not properly deleted. To resolve this, I updated the logout process by ensuring that all relevant cookies — including JWT, csrftoken, and sessionid — are properly cleared. The solution involved implementing cookie removal with the correct attributes (such as path, domain, secure, and samesite), ensuring that both the backend and frontend fully remove authentication tokens upon logout. Additionally, I ensured that the sessionid and csrftoken cookies are included in the deletion process alongside JWT-related cookies.
These changes eliminate stale session data and ensure a clean state after logout, preventing users from being logged in across multiple sessions. The logout behavior is now consistent across browsers, and no residual authentication data remains after signing out.
